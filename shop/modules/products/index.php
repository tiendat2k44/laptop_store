...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_product'])) {
        ...
        if ($name && $price > 0 && $stock >= 0) {
            $result = $db->insert(
                "INSERT INTO products (shop_id, name, price, sale_price, stock_quantity, description, status, created_at) 
                 VALUES (:shop_id, :name, :price, :sale_price, :stock, :desc, 'active', NOW())",
                [
                    'shop_id' => $shopId,
                    'name' => $name,
                    'price' => $price,
