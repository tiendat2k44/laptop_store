...
$product = $db->queryOne(
    "SELECT id, shop_id, category_id, brand_id, name, price, sale_price, stock_quantity, description, status,
       ...
       FROM products WHERE id = :id AND shop_id = :sid",
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $errors = [];
    if (!$name || $price <= 0 || $stock < 0 || $categoryId <= 0 || $brandId <= 0) {
        $errors[] = 'Vui lòng điền đầy đủ thông tin bắt buộc và chọn danh mục, thương hiệu';
    }
    ...
    if (empty($errors)) {
        try {
            ...
            $db->execute(
                "UPDATE products
                 SET name = :name, price = :price, sale_price = :salePrice, stock_quantity = :stock,
                     description = :desc, status = :status, category_id = :cid, brand_id = :bid,
            ...
            // Sắp xếp lại thứ tự ảnh chi tiết
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6 d-flex align-items-end">
                                <div class="form-check mt-2">
                                    <input class="form-check-input" type="checkbox" name="featured" id="featuredShopEdit" <?= $product['featured'] ? 'checked' : '' ?>>
                                    <label class="form-check-label" for="featuredShopEdit">Đánh dấu nổi bật</label>
                                </div>
                            </div>
                        </div>
                                            ...
                                            <div class="col-6 col-md-4 col-lg-3 text-center">
                                                <div class="border rounded p-2 h-100">
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="remove_images[]" value="<?= (int)$g['id'] ?>" id="rmshop<?= (int)$g['id'] ?>">
                                                        <label class="form-check-label" for="rmshop<?= (int)$g['id'] ?>">Xóa ảnh</label>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
